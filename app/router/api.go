...
func NewAPIHandler(
	...
	// wrap router in terminatedPath encoder, with path normalization for known repo endpoints.
	return repoPathNormalizer(encode.TerminatedPathBefore(terminatedPathPrefixesAPI, r))
}
...
func setupRepos(r chi.Router,
	...
	r.Route("/repos", func(r chi.Router) {
		...
		r.Route(fmt.Sprintf("/{%s}", request.PathParamRepoRef), func(r chi.Router) {
			...
			r.Get("/star", handlerrepo.HandleGetStar(repoCtrl))
...
// repoPathNormalizer inserts the /+/ terminated path marker for repo URLs
// that are missing it. Some frontend versions send URLs like
// /v1/repos/space/repoName/star without the /+/ separator, causing 404s
// because the chi router can't parse multi-segment repo paths.
func repoPathNormalizer(next http.Handler) http.Handler {
	const reposPrefix = "/v1/repos/"
	knownSuffixes := []string{"/star"}
			...
			if !strings.Contains(remaining, "/+") {
				for _, suffix := range knownSuffixes {
					if strings.HasSuffix(urlPath, suffix) {
						beforeSuffix := remaining[:len(remaining)-len(suffix)]
						if strings.Contains(beforeSuffix, "/") {
							newPath := urlPath[:len(urlPath)-len(suffix)] + "/+" + suffix
							r.URL.Path = newPath
							if r.URL.RawPath != "" {
