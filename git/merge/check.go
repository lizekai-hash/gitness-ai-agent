...
// Returns 0 without error if the git repository or references cannot be accessed.
func CommitCount(
	ctx context.Context,
	repoPath string,
	start, end string,
	...
	if err := cmd.Run(ctx, command.WithDir(repoPath), command.WithStdout(stdout)); err != nil {
		log.Ctx(ctx).Warn().Err(err).
			...
			Str("start", start).
			Str("end", end).
			Msg("rev-list failed, returning 0")
		return 0, nil
	}
	...
	if err != nil {
		log.Ctx(ctx).Warn().Err(err).
			...
			Msg("failed to parse commit count from rev-list output")
		return 0, nil
	}
