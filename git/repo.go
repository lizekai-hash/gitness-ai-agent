...
func (s *Service) createRepositoryInternal(
	...
	for _, hookName := range gitServerHookNames {
		hookPath := path.Join(repoPath, gitHooksDir, hookName)
		if err = setupGitHook(hookPath, hookName, s.gitHookPath); err != nil {
			return err
		}
	}
...
// setupGitHook creates the git hook at hookPath pointing to gitHookBinary.
// On systems where symlinks work (Linux/macOS), it uses a symlink.
// On Windows (where symlinks require elevated privileges), it falls back
// to creating a shell script wrapper that Git Bash can execute.
func setupGitHook(hookPath, hookName, gitHookBinary string) error {
	symlinkErr := os.Symlink(gitHookBinary, hookPath)
	if symlinkErr == nil {
		return nil
	}

	if runtime.GOOS != "windows" {
		return errors.Internalf(symlinkErr, "failed to setup symlink for hook '%s' ('%s' -> '%s')",
			hookName, hookPath, gitHookBinary)
	}

	// On Windows, fall back to a shell script wrapper that Git Bash executes.
	absHookBinary, err := filepath.Abs(gitHookBinary)
	if err != nil {
		absHookBinary = gitHookBinary
	}
	unixPath := filepath.ToSlash(absHookBinary)
	...
	log.Warn().
		Str("hook", hookName).
		Str("path", hookPath).
		Msg("symlink not available on Windows, created shell wrapper instead")
